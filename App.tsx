...
const App: React.FC = () => {
  ...
  // Auto-scroll to results when analysis completes - using requestAnimationFrame for better layout timing
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen pb-20 bg-[#fcfdfe]">
      {/* Dossier Print Header - only visible when printing */}
      <header className="hidden print:block mb-10 border-b-4 border-slate-900 pb-6 w-full">
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-4xl font-black uppercase tracking-tighter text-slate-900">Official Analysis Dossier</h1>
            <p className="text-sm font-black uppercase text-slate-500 tracking-[0.2em] mt-1">Generated by NOVA Strategic Synthesis • {new Date().toLocaleDateString()} • {new Date().toLocaleTimeString()}</p>
          </div>
          <div className="text-right">
            <div className="text-[10px] font-black uppercase text-slate-400">Security Clearance</div>
            <div className="text-xs font-black text-rose-600 uppercase">INTERNAL SCIENTIFIC USE ONLY</div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 pt-10">
        {!currentProtein ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-10 duration-700 grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <section className="lg:col-span-8 space-y-10 pb-20 w-full">
      ...
      {/* Dossier Print Footer - only visible when printing */}
      <footer className="hidden print:block mt-20 pt-10 border-t border-slate-200 w-full">
        <div className="grid grid-cols-2 gap-10">
          <div>
            ...
            <p className="text-[9px] text-slate-500 leading-relaxed italic max-w-lg">
