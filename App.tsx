...
// APP_VERSION for session cache busting if schema changes
...
const App: React.FC = () => {
  ...
  const logEvent = (feature: string, details: string) => {
    console.debug(`[EVENT] ${feature}: ${details}`);
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      logEvent('PREDICTION_SUCCESS', `Analysis complete for ${mutation.wildtype}${mutation.position}${mutation.mutant}`);
      ...
      console.error("Prediction Fault:", err);
      setError(`Analysis Error: ${err.message || "The synthesis engine failed to process the molecular request."}`);
  ...
  const handleCustomSearch = async () => {
    ...
    try {
      ...
      setError(`Resolution Fault: ${err.message || "Protein not found in public databases."}`);
    } finally {
      ...
      <nav className="bg-[#0f172a] text-white sticky top-0 z-[60] shadow-2xl px-6 py-4 border-b border-indigo-500/20">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex gap-3">
             <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="text-rose-400 hover:text-rose-300 text-[10px] font-black uppercase px-4 py-2 rounded-xl hover:bg-rose-500/10 transition-all">Reset Session</button>
             {logEntries.length > 0 && (
      ...
      {showSuccessToast && result && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[100] animate-in slide-in-from-top-4 fade-in duration-500">
           <div className="bg-indigo-600 text-white px-10 py-5 rounded-[2.5rem] shadow-[0_35px_60px_-15px_rgba(79,70,229,0.5)] border-4 border-white/20 flex items-center gap-8">
              ...
              <button onClick={() => setShowSuccessToast(false)} className="opacity-40 hover:opacity-100 transition-opacity text-white"><i className="fa-solid fa-xmark text-xl"></i></button>
           </div>
        </div>
      )}
